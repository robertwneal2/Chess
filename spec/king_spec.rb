...
describe King do
  ...
  describe '#possible_moves' do
    ...
    context 'when in middle of board surround by empty spaces' do
      subject(:king_empty) { described_class.new(:white, [4, 4]) }
      ...
      it 'returns correct 8 moves' do
        ...
        result = king_empty.possible_moves(board)
        expect(result).to eq(expected_result)
      end
    end

    context 'when in corner surrounded by empty spaces' do
      subject(:king_empty_corner) { described_class.new(:white, [0, 0]) }
      let(:board) { double('board') }
      let(:piece) { double('piece', color: :null) }
      ...
      it 'returns correct 3 moves' do
        ...
        result = king_empty_corner.possible_moves(board)
        expect(result).to eq(expected_result)
      end
    end

    context 'when in corner surrounded by white pieces' do
      subject(:king_empty_corner_white) { described_class.new(:white, [0, 0]) }
      let(:board) { double('board') }
      ...
      it 'returns no moves' do
        allow(board).to receive(:[]).and_return(piece)
        expected_result = []
        result = king_empty_corner_white.possible_moves(board)
        expect(result).to eq(expected_result)
      end
    end

    context 'when in corner surrounded by black pieces' do
      subject(:king_empty_corner_black) { described_class.new(:white, [0, 0]) }
      let(:board) { double('board') }
      ...
      it 'returns correct 3 moves' do
        ...
        result = king_empty_corner_black.possible_moves(board)
        expect(result).to eq(expected_result)
      end
    end
